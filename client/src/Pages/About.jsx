...
function About() {
  ...
  const [showReadMore, setShowReadMore] = useState(Array(4).fill(true));
  ...
  const handleReadMore = (index) => {
    if (expandedCard === index) {
      ...
      setShowReadMore(showReadMore.map((_, i) => (i === index ? false : true)));
    }
  };
  ...
  useEffect(() => {
    ...
    if (expandedCard !== null) {
      timeout = setTimeout(() => {
        setExpandedCard(null);
        setShowReadMore(showReadMore.map((_, i) => (i === expandedCard ? true : showReadMore[i])));
      ...
      <div>
        ...
        <div className="flex flex-col gap-5 items-center justify-between w-full">
          {teamMembers.map((member, index) => (
            <div key={index} className="container-card" data-aos={index % 2 === 0 ? "fade-right" : "fade-left"}>
              ...
              {expandedCard === index ? (
                ...
                showReadMore[index] && (
                  <button onClick={() => handleReadMore(index)} className="msg-btn">Read more</button>
                )
              )}
              <div>
